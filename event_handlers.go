...
func logOnEvent(event Event) {
	...
	case ErrorEvent:
		log.Println("Splunk Tracer error: ", event)
	default:
		log.Println("Splunk Tracer event: ", event)
	}
}
...
func (l *logOneError) OnEvent(event Event) {
	...
	case ErrorEvent:
		l.Once.Do(func() {
			log.Printf("Splunk Tracer error: (%s). NOTE: Set the SetGlobalEventHandler handler to log events.\n", event.Error())
		})
	}
}
