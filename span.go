...
ReferencesLoop:
	...
	if tracer.opts.MetaEventReportingEnabled && !sp.IsMeta() {
		opentracing.StartSpan(SPLMetaEvent_SpanStartOperation,
			opentracing.Tag{Key: SPLMetaEvent_MetaEventKey, Value: true},
			opentracing.Tag{Key: SPLMetaEvent_SpanIdKey, Value: sp.raw.Context.SpanID},
			opentracing.Tag{Key: SPLMetaEvent_TraceIdKey, Value: sp.raw.Context.TraceID}).
			Finish()
	}
	return sp
}
...
func (s *spanImpl) FinishWithOptions(opts opentracing.FinishOptions) {
	...
	if s.tracer.opts.MetaEventReportingEnabled && !s.IsMeta() {
		opentracing.StartSpan(SPLMetaEvent_SpanFinishOperation,
			opentracing.Tag{Key: SPLMetaEvent_MetaEventKey, Value: true},
			opentracing.Tag{Key: SPLMetaEvent_SpanIdKey, Value: s.raw.Context.SpanID},
			opentracing.Tag{Key: SPLMetaEvent_TraceIdKey, Value: s.raw.Context.TraceID}).
			Finish()
	}
}
...
func (s *spanImpl) IsMeta() bool {
	return s.raw.Tags["splunktracing.meta_event"] != nil
}
