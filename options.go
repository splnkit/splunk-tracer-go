...
const (
	DefaultCollectorPath     = "/services/collector/events"
	DefaultPlainPort         = 8088
	DefaultSecurePort        = 8088
...
const (
	...
	TracerPlatformVersionKey = "tracer_platform_version"
	TracerVersionKey         = "tracer_version" // Note: TracerVersionValue is generated from ./VERSION
)
...
type Options struct {
	// AccessToken is the unique API key for your Splunk project.  It is
	// available on your account page at https://app.splunk.com/account
	AccessToken string `yaml:"access_token" usage:"access token for reporting to Splunk"`
	...
	// Splunk is the host, port, and plaintext option to use
	// for the Splunk HEC API.
	SplunkAPI Endpoint `yaml:"splunk_hec_api"`
	...
	// sent by a client.
	MaxCallSendMsgSizeBytes int `yaml:"max_call_send_msg_size_bytes"`
	...
	// DEPRECATED: The Splunk library prints the first error to stdout by default.
	...
	// Enable Splunk Meta Event Logging
...
func (opts *Options) Initialize() error {
	...
	if opts.Collector.Host == "" {
		opts.Collector.Host = DefaultCollectorHost
	}
...
func (sid SetSpanID) applySPL(sso *startSpanOptions) {
...
func (sid SetTraceID) applySPL(sso *startSpanOptions) {
...
func (sid SetParentSpanID) applySPL(sso *startSpanOptions) {
	sso.SetParentSpanID = uint64(sid)
}

// splunkStartSpanOption is used to identify splunk-specific Span options.
type splunkStartSpanOption interface {
	applySPL(*startSpanOptions)
}
...
type startSpanOptions struct {
	...
	// from another system into Splunk via opentracing APIs.
	SetSpanID       uint64
