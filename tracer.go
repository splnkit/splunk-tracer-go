...
// tracer and typecast it to a `splunktracing.Tracer`. As a convenience, the
// splunktracing package provides static functions which perform the typecasting.
type Tracer interface {
	opentracing.Tracer

	// Close flushes and then terminates the Splunk collector
...
// Implements the `Tracer` interface. Buffers spans and forwards to a Splunk collector.
...
// NewTracer creates and starts a new Splunk Tracer.
...
func (tracer *tracerImpl) Inject(sc opentracing.SpanContext, format interface{}, carrier interface{}) error {
	if tracer.opts.MetaEventReportingEnabled {
		opentracing.StartSpan(SPLMetaEvent_InjectOperation,
			opentracing.Tag{Key: SPLMetaEvent_MetaEventKey, Value: true},
			opentracing.Tag{Key: SPLMetaEvent_TraceIdKey, Value: sc.(SpanContext).TraceID},
			opentracing.Tag{Key: SPLMetaEvent_SpanIdKey, Value: sc.(SpanContext).SpanID},
			opentracing.Tag{Key: SPLMetaEvent_PropagationFormatKey, Value: format}).
			Finish()
	}
	switch format {
...
func (tracer *tracerImpl) Extract(format interface{}, carrier interface{}) (opentracing.SpanContext, error) {
	if tracer.opts.MetaEventReportingEnabled {
		opentracing.StartSpan(SPLMetaEvent_ExtractOperation,
			opentracing.Tag{Key: SPLMetaEvent_MetaEventKey, Value: true},
			opentracing.Tag{Key: SPLMetaEvent_PropagationFormatKey, Value: format}).
			Finish()
	}
...
func (tracer *tracerImpl) Flush(ctx context.Context) {
	...
	if tracer.opts.MetaEventReportingEnabled && !tracer.firstReportHasRun {
		opentracing.StartSpan(SPLMetaEvent_TracerCreateOperation,
			opentracing.Tag{Key: SPLMetaEvent_MetaEventKey, Value: true},
			opentracing.Tag{Key: SPLMetaEvent_TracerGuidKey, Value: tracer.reporterID}).
			Finish()
